...
- running the resilienceOS AI resilience agent workflow
- installing or validating this repository as a local Codex skill
- running CLI smoke checks and output behavior checks
...
4. `resilienceos agent --scenario singapore --include-inbox --include-simulate --format json`
5. `resilienceos assess --format xml`
   - expect a clean CLI failure for invalid format
6. `resilienceos assess --input fixtures/scenario_singapore_coastal.json --format json`
